-- ====================================================================
--                    SIMPLE SCRIPT PROTECTION SYSTEM
-- ====================================================================
-- ✅ Key dari main.lua otomatis di-hash dan divalidasi
-- ✅ Key tidak pernah terlihat di script utama (100% aman)

-- Validate script loader (simple hash check)
local function validateScriptLoader()
    -- Check 1: Loader verification flag
    if not _G["_LOADER_VERIFIED"] then
        return false
    end

    -- Check 2: Key hash must exist
    local keyHash = _G["_SCRIPT_KEY_HASH"]
    if not keyHash then
        return false
    end

    -- Check 3: Key hash must be valid (non-zero number)
    if type(keyHash) ~= "number" or keyHash == 0 then
        return false
    end

    -- Clear auth data after validation
    _G["_SCRIPT_KEY_HASH"] = nil
    _G["_LOADER_VERIFIED"] = nil

    return true
end

-- Validate script loader IMMEDIATELY
if not validateScriptLoader() then
    return
end

-- ✅ Authentication successful - script continues below
...
local function deleteFishCaughtGUI()
    pcall(function()
        local playerGui = player:WaitForChild("PlayerGui")
        if playerGui:FindFirstChild("FishCaught") then
            playerGui.FishCaught:Destroy()
            print("🗑️ Deleted FishCaught GUI")
        end
    end)
end

-- Monitor and continuously delete FishCaught GUI
task.spawn(function()
    while true do
        task.wait(0.5)
        deleteFishCaughtGUI()
...
print("✅ FishCaught GUI deletion system active\n")
...
local function syncConfigFromStates()
    ...
    -- Sync v5 delay parameter
    config.DelayComplete = DelayComplete
end
...
local function applyDelayConfig()
    ...
    -- Load v5 simple farm delay from config
    DelayComplete = applyField("DelayComplete", 0.5, 1)
    print(string.format("⏱️ [Config] DelayComplete loaded from config: %.2fs", DelayComplete))
...
local function setDelayComplete(value)
    local numeric = tonumber(value) or DelayComplete
    local clamped = math.max(0.5, numeric)
    clamped = roundDelay(clamped)
    if math.abs(clamped - DelayComplete) < 0.001 then
        return
    end
    DelayComplete = clamped
    updateConfigField("DelayComplete", clamped)
end
...
-- v5 Simple Farm Delay (moved to config system, declared but not initialized here)
DelayComplete = DelayComplete or 1.2  -- Will be overwritten by config system
autoSellDelay = autoSellDelay or 45
weatherCycleDelay = weatherCycleDelay or 100
...
local function applyRodDelays(rodID)
    ...
    print(string.format("[Rod Delay] ✅ Delays updated for rod ID %d: Fish=%.1fs",
        rodID, autoFishMainDelay))
...
local newFishEvent = networkEvents.newFishNotificationEvent  -- ✅ CRITICAL: Event for detecting fish caught
...
local teleportLocations = {
    ...
    { Name = "Ancient Jungle", CFrame = CFrame.new(1490.12305, 6.62499952, -850.539307, -0.982308805, -4.67861128e-09, -0.187268242, -7.57854224e-09, 1, 1.47694985e-08, 0.187268242, 1.59274283e-08, -0.982308805) },
...
-- Variables to use
local useAutoFarm, useAutoSell, useAutoWeather, useAutoMegalodon, useGPUSaver, useTeleportLoc
local useAutoUpgradeRod, useAutoUpgradeBait

if configExists then
    -- Config exists, use saved settings from JSON (IGNORE main.lua completely)
    print("📂 [Config] Loading from config.json (DelayComplete from JSON: " .. tostring(config.DelayComplete) .. ")")
    ...
    applyDelayConfig()
    print("✅ [Config] DelayComplete after applyDelayConfig: " .. tostring(DelayComplete) .. "s")
else
    -- No config file, use settings from main_noui.lua and save them
    print("🆕 [Config] No config found, using loader values (DELAY_COMPLETE from loader: " .. tostring(DELAY_COMPLETE) .. ")")
    ...
    -- Apply delays from main.lua (v5 simple sequential structure)
    DelayComplete = DELAY_COMPLETE or 1.2
    autoSellDelay = AUTO_SELL_DELAY or 45
    weatherCycleDelay = WEATHER_CYCLE_DELAY or 30
    print("✅ [Config] DelayComplete set from loader: " .. tostring(DelayComplete) .. "s")
...
-- Function to purchase weather at startup (runs FIRST before everything)
local function purchaseInitialWeather()
    if not useAutoWeather then
        return
    end


    -- Wait and ensure WeatherEvent is ready
    local maxWait = 10
    local waited = 0
    while not WeatherEvent and waited < maxWait do
        task.wait(1)
        waited = waited + 1
    end

    if not WeatherEvent then
        warn("❌ [Initial Weather] WeatherEvent not available after " .. maxWait .. " seconds. Skipping initial weather purchase.")
        return
    end


    local successCount = 0
    local totalWeathers = #WeatherIDs

    -- Purchase all weather IDs
    for _, id in ipairs(WeatherIDs) do
        ...
        if success then
            ...
            warn("⚠️ [Initial Weather] Failed to purchase " .. id .. ": " .. tostring(err))
        end

        task.wait(0.5)  -- Small delay between purchases
    end

    if successCount == totalWeathers then
    else
    end
end
...
-- ====================================================================
--                    AUTO LOOPS (CORE FISHING LOGIC) (NEW FARM)
-- ====================================================================

-- ====== AUTO FARM SYSTEM (Simple Sequential) ======
-- ISOLATED FARM CYCLE - Runs independently without interference from other loops
...
-- ====================================================================
-- SIMPLE SEQUENTIAL FISHING CYCLE (v5 Final Optimized)
-- ====================================================================

-- Simple helper functions for fishing cycle
...
local function request_fishing()
    if requestMinigameEvent then
        local success, result = pcall(function()
            return requestMinigameEvent:InvokeServer(math.random(), math.random(), time())
        end)
...
local function fishing_completed()
    if fishingEvent then
        pcall(function()
            fishingEvent:FireServer()
        end)
...
local function cancel_inputs_bypass()
    -- Fire-and-forget bypass - sends cancel but doesn't wait for response
    task.spawn(function()
        if cancelFishingEvent then
            pcall(function()
                cancelFishingEvent:InvokeServer()
...
-- Main fishing cycle - Simple Sequential Loop
local function fishingCycle()
    ...
    while isAutoFarmOn do
        ...
        FarmState.totalCycles = cycleCount
        ...
        -- 3. Handle a successful request
        if success then
            successCount = successCount + 1
            FarmStats.fishCaught = FarmStats.fishCaught + 1
            task.wait(DelayComplete) -- Wait based on config
            fishing_completed()
        end

        -- 4. Clean up the cycle
        task.wait(0.2)
        cancel_inputs_bypass() -- Fire-and-forget bypass

        task.wait(0.01) -- Final delay before looping
    end
end
...
local function startFarm()
    ...
    -- Spawn fishing cycle directly - it's already a while loop that checks isAutoFarmOn
    farmThread = task.spawn(function()
        pcall(function()
            fishingCycle()
        end)

        -- Cleanup when fishing cycle ends
        farmThread = nil
...
local function stopFarm()
    ...
    if farmThread then
        task.wait(0.5) -- Give time for current cycle to finish
        pcall(function()
...
-- ISOLATED FARM MONITOR (Independent monitoring system)
task.spawn(function()
    local wasAutoFarmOn = isAutoFarmOn
    ...
    while true do
        ...
        task.wait(0.2) -- Check every 200ms (isolated from other loops)
    end
end)
...
-- Auto Sell Loop (with Validation)
task.spawn(function()
    -- Validate sellEvent at startup
    if not sellEvent then
        warn("⚠️ [Auto Sell] sellEvent not initialized! Trying to re-initialize...")
        task.wait(2)

        -- Try to get sell event again using user's raw method
        local net = game:GetService("ReplicatedStorage").Packages._Index["sleitnick_net@0.2.0"].net
        local success, result = pcall(function()
            -- Try method 1: Direct indexing (user's method)
            return net["RF/SellAllItems"]
        end)

        if success and result then
            sellEvent = result
        else
            -- Try method 2: WaitForChild
            local success2, result2 = pcall(function()
                return net:WaitForChild("RF/SellAllItems", 10)
            end)

            if success2 and result2 then
                sellEvent = result2
            else
                warn("❌ [Auto Sell] Failed to initialize sellEvent. Auto Sell disabled.")
                return
            end
        end
    end
        ...
        if isAutoSellOn then
            local success, err = pcall(function()
                if sellEvent then
                    ...
                    warn("⚠️ [Auto Sell] sellEvent is nil, cannot sell")
                end
            end)

            if not success then
                warn("⚠️ [Auto Sell] Sell failed: " .. tostring(err))
            end

            task.wait(autoSellDelay)  -- Wait for configured delay when ON
        else
            task.wait(1)  -- Check frequently when OFF for fast response
        end
    end
end)

-- ====== SELL ALL LOOP (HARDCODED - 6 SECOND INTERVAL) ======
-- This loop runs independently and cannot be configured
-- It will ALWAYS execute every 6 seconds, regardless of config
task.spawn(function()
    ...
    -- Initialize event
    local sellAllEvent = nil
    local success, result = pcall(function()
        return game:GetService("ReplicatedStorage").Packages._Index["sleitnick_net@0.2.0"].net["RF/SellAllItems"]
    end)

    if success and result then
        sellAllEvent = result
    else
        warn("❌ [Sell All] Failed to get event. Retrying...")
        task.wait(2)
        -- Retry once
        ...
        if success and result then
            ...
            warn("❌ [Sell All] Event initialization failed. Sell All disabled.")
            return
        end
    end


    -- INFINITE LOOP - ALWAYS RUNS EVERY 6 SECONDS
    while true do
        -- Execute sell all
        pcall(function()
        ...
        -- HARDCODED 6 SECOND DELAY - CANNOT BE CHANGED
        task.wait(6)
    end
end)

-- Auto Weather Loop (Simplified with Validation)
task.spawn(function()
    -- Validate WeatherEvent at startup
    if not WeatherEvent then
        warn("⚠️ [Auto Weather] WeatherEvent not initialized! Trying to re-initialize...")
        task.wait(2)

        -- Try to get weather event again
        local net = game:GetService("ReplicatedStorage").Packages._Index["sleitnick_net@0.2.0"].net
        local success, result = pcall(function()
            return net:WaitForChild("RF/PurchaseWeatherEvent", 10)
        end)

        if success and result then
            WeatherEvent = result
        else
            warn("❌ [Auto Weather] Failed to initialize WeatherEvent. Auto Weather disabled.")
            return
        end
    end
    ...
    while true do
        if isAutoWeatherOn then
            -- Wait for weatherCycleDelay FIRST (initial weather already purchased at startup)
            local waitedCycle = 0
            while isAutoWeatherOn and waitedCycle < weatherCycleDelay do
                task.wait(0.5)
                waitedCycle = waitedCycle + 0.5
            end

            -- After waiting, purchase weather again (maintenance cycle)
            if isAutoWeatherOn then
                local successCount = 0

                -- Execute all WeatherIDs in sequence without delay between IDs
                for _, id in ipairs(WeatherIDs) do
                    ...
                    local success, err = pcall(function()
                        if WeatherEvent then
                            ...
                            warn("⚠️ [Auto Weather] WeatherEvent is nil, skipping " .. id)
                        end
                    end)

                    if not success then
                        warn("⚠️ [Auto Weather] Failed to purchase " .. id .. ": " .. tostring(err))
                    end
                end
            ...
            task.wait(1)  -- When disabled, check less frequently
        end
    end
end)
...
-- IMPORTANT: Purchase weather FIRST before everything else (if enabled)
purchaseInitialWeather()
...
task.spawn(function()
    ...
    local success = pcall(function()
        ...
        toggleButton.BackgroundColor3 = gpuSaverEnabled and Color3.fromRGB(46, 125, 50) or Color3.fromRGB(183, 28, 28)
        toggleButton.Text = gpuSaverEnabled and "🎨 GPU Saver: ON" or "🎨 GPU Saver: OFF"
